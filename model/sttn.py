...
class BaseNetwork(nn.Module):   ## 这是一个父类  用来存一些通用函数
    def __init__(self):
        super(BaseNetwork, self).__init__()

    def print_network(self):   ## 打印模型参数数量
    ...
    def init_weights(self, init_type='normal', gain=0.02):   ##  初始化权重
        '''
        ...
        def init_func(m):
            ...
            if classname.find('InstanceNorm2d') != -1:   ## -1表示没找到 也就是instanceNorm层的初始化方法
                if hasattr(m, 'weight') and m.weight is not None:   ## 是否有weight属性  并且属性不能为空
                    nn.init.constant_(m.weight.data, 1.0)  ## 此时初始化为常量1.0
                if hasattr(m, 'bias') and m.bias is not None:  ## 偏置初始化为0
                    nn.init.constant_(m.bias.data, 0.0)
            elif hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):  ## 卷积层和全连接层
                if init_type == 'normal':
        ...
        self.apply(init_func)  ## 自己和子类递归调用
...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value, m):
        ...
        scores.masked_fill(m, -1e9 if scores.dtype == torch.float32 else -1e4)
