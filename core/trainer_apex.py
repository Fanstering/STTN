...
class Trainer():
    def __init__(self, config, debug=False):
        ...
        [self.netG, self.netD], [self.optimG, self.optimD] = amp.initialize([self.netG, self.netD],
                                                                            [self.optimG, self.optimD], opt_level="O2",
                                                                            num_losses=2)
    ...
    def _train_epoch(self, pbar):
        ...
        # scaler = GradScaler(enabled=self.config['trainer']['mixed_precision'])
        for frames, masks in self.train_loader:
            ...
            # dis_loss.backward()
            with amp.scale_loss(dis_loss, self.optimD, loss_id=0) as errD_scaled:
                errD_scaled.backward()
            self.optimD.step()

## TypeError: 'Discriminator' object is not iterable 这个报错是因为scale_loss函数的第二个参数应该写优化器，写成了模型
            ...
            # gen_loss.backward()
            with amp.scale_loss(gen_loss, self.optimG, loss_id=1) as errG_scaled:
